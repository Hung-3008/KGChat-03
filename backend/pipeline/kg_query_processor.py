...
async def retrieve_and_format_kg_results(
    ...
    Args:
        ...
        similarity_threshold: Similarity threshold for filtering nodes
...
class KnowledgeGraphQueryProcessor:
    ...
    async def process_query(
        ...
        try:
            ...
            if intent == QueryIntent.GREETING:
                ...
                # For medical queries, retrieve from knowledge graph and generate response
                ...
                if high_level_keywords or low_level_keywords:
                    try:
                        ...
                        result["response"] = await self._generate_healthcare_response_with_rag(
    ...
    async def _generate_healthcare_response(
        ...
        # Get prompt template from pipeline_prompts
        prompt_template = PROMPTS.get("healthcare_response", "")
        prompt = prompt_template.format(
            query=query,
            context=context,
        ...
        try:
            ...
            answer = extract_llm_response(response)
            if not answer:
                return "I don't have enough information about that specific medical topic in my knowledge sources. Would you like to ask about something else related to healthcare?"
            return answer
    ...
    async def _generate_healthcare_response_with_rag(
        self,
        rag_prompt: str,
        response_type: str = "concise"
    ) -> str:
        """
        Generate a healthcare response using RAG prompt.
