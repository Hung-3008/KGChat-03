...
def save_llm_answer(
    ...
    try:
        ...
        # Keywords (if healthcare related)
        if result.get('intent') == QueryIntent.HEALTHCARE_RELATED or (
            isinstance(result.get('intent'), str) and
            result.get('intent') == QueryIntent.HEALTHCARE_RELATED.value
        ):
...
async def process_kg_query(
    ...
    Args:
        ...
        embedding_client: Embedding client for generating embeddings (e.g., TransformerEncoder)
...
async def process_query_full_pipeline(
    ...
    2. Keyword extraction (if HEALTHCARE_RELATED)
    3. Knowledge graph retrieval (Level 1 and Level 2)
    ...
    Args:
        ...
        clients: Dictionary containing all necessary clients:
            ...
            - neo4j_client: Neo4j database client
            - embedding_client: Embedding client (e.g., TransformerEncoder)
            - qdrant_client: Qdrant vector database client
    ...
    # STEP 2: Keyword extraction (only for HEALTHCARE_RELATED)
    if intent == QueryIntent.HEALTHCARE_RELATED:
        ...
        if high_level_keywords or low_level_keywords:
            ...
            embedding_client = clients.get("ollama_client")
            qdrant_client = clients.get("qdrant_client")

            if not neo4j_client or not embedding_client or not qdrant_client:
                logger.error(
                    "Missing required clients for knowledge graph query")
                logger.error(f"   - Neo4j: {'✅' if neo4j_client else '❌'}")
                logger.error(
                    f"   - Embedding Client: {'✅' if embedding_client else '❌'}")
                logger.error(f"   - Qdrant: {'✅' if qdrant_client else '❌'}")
                return result
        ...
        logger.info("   (Only processing for HEALTHCARE_RELATED)")
